      ...
      {insights.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {insights.map((insight, index) => (
            <div
              key={index}
              className={`p-6 rounded-3xl border ${getBgColor(insight.type)}`}
            >
        ...
        <div className="bg-white dark:bg-gray-900 p-8 rounded-3xl text-center border border-gray-100 dark:border-gray-800">
          ...
          <p className="text-sm text-gray-400 mt-1">Add some transactions to unlock personalized insights</p>
        </div>
      )}
    </div>
  );
